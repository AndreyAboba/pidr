    ...
    AutoShootStatus.Connection = RunService.Heartbeat:Connect(function()
        ...
        if hasBall and TargetPoint and dist <= AutoShootConfig.MaxDistance then
            if not AutoShootStatus.ManualShot then
                ...
                if success then
                    LastShoot = tick()
                    notify("AutoShoot", "AUTO SHOT! [" .. CurrentType .. "]", true)
                end
            end
        end
    end)
        ...
        if inp.KeyCode == AutoShootStatus.Key then
            ...
            if hasBall and TargetPoint then
                ...
                if ShootDir then
                    ...
                    if success then
                        notify("AutoShoot", "MANUAL SHOT! [" .. CurrentType .. "]", true)
                        LastShoot = tick()
                        CanShoot = false
...
AutoPickup.Start = function()
    ...
    AutoPickupStatus.Connection = RunService.Heartbeat:Connect(function()
        if not AutoPickupConfig.Enabled then return end
        local ball = Workspace:FindFirstChild("ball")
        if not ball or ball:FindFirstChild("playerWeld") or (HumanoidRootPart.Position - ball.Position).Magnitude > AutoPickupConfig.PickupDist then return end
        if PickupRemote then pcall(function() PickupRemote:FireServer(AutoPickupConfig.SpoofValue) end) end
    end)

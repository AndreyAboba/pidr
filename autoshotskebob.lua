-- [v35.43] AUTO SHOOT + FULL UI (всё в одном)
-- UI: Toggle, Slider, Keybind, Notify, Sync
-- Кейбинд отображается в GUI
-- Атаки включаются/выключаются
...
local AnimationHoldTime = 0.6  -- ДЛИТЕЛЬНОСТЬ АНИМАЦИИ

-- === НАСТРОЙКИ (ДИНАМИЧЕСКИЕ ЧЕРЕЗ UI) ===
local Settings = {
    Enabled = true,
    AutoPickup = true,
    PickupDist = 180,
    ...
    -- РЕЖИМЫ
    Legit = true,          -- ВКЛ/ВЫКЛ анимацию
    ManualShot = true,     -- true = только по клавише, false = авто
    ShootKey = Enum.KeyCode.E,  -- ← МЕНЯЙ ЗДЕСЬ, будет отображаться в GUI
...
-- === ОБНОВЛЕНИЕ GUI РЕЖИМА ===
local function UpdateModeText()
    if Settings.ManualShot then
        Gui.Mode.Text = string.format("Mode: Manual (%s)", GetKeyName(Settings.ShootKey))
    else
...
local function CalculateTrajectoryHeight(dist, power, attackName, isLowShot)
    local cfg = Settings.Attacks[attackName] or {}
    ...
    local gravityFall = attackName == "FarSmartCandle" and 10 or 0.5 * Settings.Gravity * timeToTarget^2
    local height = math.clamp(baseHeight + gravityFall, isLowShot and 0.5 or 2.0, Settings.MaxHeight)
    if power < 1.5 and attackName ~= "FarSmartCandle" then height = math.clamp(height * (power / 1.5), isLowShot and 0.5 or 2.0, height) end
    height = math.clamp(height * heightMult, isLowShot and 0.5 or 2.0, Settings.MaxHeight)
...
local function GetTarget(dist, goalieX, goalieY, isAggressive, goaliePos, playerAngle)
    ...
    if dist > Settings.MaxDistance then return nil, "None", "None", 0 end

    local startPos = HumanoidRootPart.Position
    local halfWidth = (GoalWidth / 2) - Settings.Inset
    local halfHeight = (GoalHeight / 2) - Settings.Inset
    ...
    for name, cfg in pairs(Settings.Attacks) do
        if not cfg.Enabled or dist < cfg.MinDist or dist > math.min(cfg.MaxDist, Settings.MaxDistance) then continue end
        ...
        for _, target in ipairs(targets) do
            ...
            local power = cfg.Power or math.clamp(Settings.MinPower + dist * Settings.PowerPerStud, cfg.PowerMin or Settings.MinPower, Settings.MaxPower)
            ...
            if heightAdjust > 0 then height = math.clamp(heightAdjust, 2.0, Settings.MaxHeight)
    ...
    if #candidates == 0 then
        ...
        local power = math.clamp(Settings.MinPower + dist * Settings.PowerPerStud, Settings.MinPower, Settings.MaxPower)
...
local function CalculateTarget()
    ...
    Gui.Dist.Text = string.format("Dist: %.1f (Max: %.1f)", dist, Settings.MaxDistance)
    if dist > Settings.MaxDistance then TargetPoint = nil; NoSpinPoint = nil; Gui.Target.Text = "Target: --"; Gui.Power.Text = "Power: --"; Gui.Spin.Text = "Spin: --"; return end
...
UserInputService.InputBegan:Connect(function(inp, gp)
    if gp or not Settings.Enabled or not Settings.ManualShot then return end
    if inp.KeyCode == Settings.ShootKey and CanShoot then
        ...
        if hasBall and TargetPoint then
            ...
            if ShootDir then
                if Settings.Legit and not IsAnimating then
                    IsAnimating = true
                    RShootAnim:Play()
...
local function AutoShoot()
    if Settings.ManualShot or not Settings.Enabled then return end
    local ball = Workspace:FindFirstChild("ball")
    ...
    local dist = (HumanoidRootPart.Position - GoalCFrame.Position).Magnitude
    if dist > Settings.MaxDistance then return end
...
RunService.Heartbeat:Connect(function()
    ...
    if hasBall and TargetPoint and dist <= Settings.MaxDistance then
        Gui.Status.Text = Settings.ManualShot and "Ready (Press " .. GetKeyName(Settings.ShootKey) .. ")" or "Aiming..."
        Gui.Status.Color = Color3.fromRGB(0,255,0)
    elseif hasBall then
        Gui.Status.Text = dist > Settings.MaxDistance and "Too Far" or "No Target"
...
RunService.Heartbeat:Connect(function()
    ...
    local ball = Workspace:FindFirstChild("ball")
    if not ball or ball:FindFirstChild("playerWeld") or (HumanoidRootPart.Position - ball.Position).Magnitude > Settings.PickupDist then return end
    if PickupRemote then pcall(function() PickupRemote:FireServer(Settings.SpoofValue) end) end
end)
...
-- === UI ИНИЦИАЛИЗАЦИЯ (ПОСЛЕДНЯЯ ЧАСТЬ) ===
local uiElements = {}
local function InitUI(UI, notify)
    if not UI.Tabs or not UI.Tabs.Combat then return end

    local section = UI.Sections.AutoShoot or UI.Tabs.Combat:Section({ Name = "AutoShoot", Side = "Left" })
    UI.Sections.AutoShoot = section

    section:Header({ Name = "AutoShoot v35.43" })
    section:SubLabel({ Text = "Smart aim + manual/auto shot" })

    uiElements.Enabled = section:Toggle({ Name = "Enabled", Default = true, Callback = function(v) Settings.Enabled = v; notify("AutoShoot", v and "Enabled" or "Disabled", v) end })
    uiElements.Legit = section:Toggle({ Name = "Legit Animation", Default = true, Callback = function(v) Settings.Legit = v end })
    uiElements.ManualShot = section:Toggle({ Name = "Manual Mode", Default = true, Callback = function(v) Settings.ManualShot = v; UpdateModeText() end })
    uiElements.ShootKey = section:Keybind({ Name = "Shoot Key", Default = Enum.KeyCode.E, Callback = function(k) Settings.ShootKey = k; UpdateModeText() end })

    section:Divider()
    uiElements.MaxDistance = section:Slider({ Name = "Max Distance", Min = 100, Max = 300, Default = 160, Callback = function(v) Settings.MaxDistance = v end })
    uiElements.AutoPickup = section:Toggle({ Name = "Auto Pickup", Default = true, Callback = function(v) Settings.AutoPickup = v end })
    uiElements.PickupDist = section:Slider({ Name = "Pickup Distance", Min = 50, Max = 300, Default = 180, Callback = function(v) Settings.PickupDist = v end })

    section:Divider()
    section:Header({ Name = "Attack Profiles" })

    for name, cfg in pairs(Settings.Attacks) do
        local atkSec = section:Section({ Name = name, Side = "Left" })
        uiElements[name .. "_Enabled"] = atkSec:Toggle({ Name = "Enabled", Default = cfg.Enabled, Callback = function(v) cfg.Enabled = v end })

        atkSec:Slider({ Name = "Min Dist", Min = 0, Max = 300, Default = cfg.MinDist, Callback = function(v) cfg.MinDist = v end })
        atkSec:Slider({ Name = "Max Dist", Min = 0, Max = 300, Default = cfg.MaxDist, Callback = function(v) cfg.MaxDist = v end })
        if cfg.Power then
            atkSec:Slider({ Name = "Power", Min = 0, Max = 100, Default = cfg.Power, Callback = function(v) cfg.Power = v end })
        elseif cfg.PowerMin then
            atkSec:Slider({ Name = "Power Min", Min = 0, Max = 100, Default = cfg.PowerMin, Callback = function(v) cfg.PowerMin = v end })
        elseif cfg.PowerAdd then
            atkSec:Slider({ Name = "Power Add", Min = -10, Max = 100, Default = cfg.PowerAdd, Callback = function(v) cfg.PowerAdd = v end })
        end
    end

    local syncSec = UI.Tabs.Config:Section({ Name = "AutoShoot Sync", Side = "Right" })
    syncSec:Button({ Name = "Sync All", Callback = function()
        for k, el in pairs(uiElements) do
            if typeof(el.GetValue) == "function" then
                local val = el:GetValue()
                if k:find("_Enabled") then
                    local atk = k:match("(.+)_Enabled")
                    if Settings.Attacks[atk] then Settings.Attacks[atk].Enabled = val end
                elseif k == "ShootKey" then Settings.ShootKey = val
                else
                    local field = k:gsub("^%u", string.lower):gsub("^%l", string.upper)
                    if Settings[field] ~= nil then Settings[field] = val end
                end
            end
        end
        UpdateModeText()
        notify("AutoShoot", "Synced!", true)
    end })
end
...
task.spawn(function()
    while task.wait(1) do
        if _G.UI and _G.notify then
            InitUI(_G.UI, _G.notify)
            break
        end
    end
end)

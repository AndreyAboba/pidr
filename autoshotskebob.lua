-- [v35.43] AUTO SHOOT + FULL UI (модуль, как TargetESP)
-- Использует UI, Core, notify из Init
-- Работает с UI.Sections.AutoShoot (уже существует)
...
function AutoShoot.Init(UI, Core, notify)
    ...
    local function UpdateModeText()
        Gui.Mode.Text = Settings.ManualShot and ("Manual (" .. GetKeyName(Settings.ShootKey) .. ")") or "Auto"
    end
    UpdateModeText()
    ...
    local function InitializeCubes()
        ...
        for _, cube in {TargetCube, GoalCube, NoSpinCube} do
            for _, line in cube do
    ...
    local function DrawOrientedCube(cube, cframe, size)
        if not cframe or not size then for _, l in cube do l.Visible = false end return end
        local half = size / 2
        local corners = {
    ...
    -- === UI (используем UI.Sections.AutoShoot) ===
    local uiElements = {}
    if UI.Sections.AutoShoot then
        local section = UI.Sections.AutoShoot

        section:Header({ Name = "AutoShoot v35.43" })
        section:SubLabel({ Text = "Smart aim + manual/auto shot" })

        uiElements.Enabled = section:Toggle({ Name = "Enabled", Default = true, Callback = function(v) Settings.Enabled = v; notify("AutoShoot", v and "ON" or "OFF", v) end })
